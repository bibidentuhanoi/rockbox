...
#else
#ifdef ESP32
#define PCMBUF_WATERMARK    (BYTERATE / 2)  /* 0.5s — smoother track transitions */
#else
...
void pcmbuf_play_start(void)
{
    ...
    int ch_status = mixer_channel_status(PCM_MIXER_CHAN_PLAYBACK);
    bool has_data = (chunk_widx != chunk_ridx);

    if (ch_status == CHANNEL_STOPPED && has_data)
    {
        current_desc = NULL;
#ifdef ESP32
        /* Force PCM restart on track transition.
           On ESP32, the FreeRTOS playback task drives mixer callbacks without
           holding pcm_mutex, so a newly activated channel may not be picked up
           by already-running callbacks. Stopping PCM first ensures
           mixer_start_pcm does a full fresh start (double-buffer init +
           pcm_play_dma_start). Safe because the playback channel is
           CHANNEL_STOPPED — only idle silence frames are interrupted. */
        if (pcm_is_playing())
            pcm_play_stop();
#endif
